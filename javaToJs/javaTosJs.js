...
$(function () {
    ...
    $('#beanBtn').click(function () {
        ...
        var javaBeanClass = beanVal.match(/import.*? public class .*/gi)[0];
        var javaBean = beanVal.match(/public class .*/gi)[0];
        // console.log(javaBean);
        // debugger;

        //获取className；public class 类名
        var classStr = javaBeanClass.match(/(\/\*\*.*?\*\/.*?public class \w+)|(\/\/.*?public class \w+)/)[0];
        var classArr = classStr.split(' ');
        //匹配字符和中文组成的注释内容
        var classComment = classStr.match(/[^\n\f\r\t\v\*\/ ]+/)[0];
        ...
        var modelJson = '    //所有Model和字段的初始化Json配置\n' +
            'var ' + ModelName + 'Json = {\n' + ModelName + ':{\n';
        ...
        // 不包含声明为List的字段
        //(/** ... */ private ... ;)或者(// ... private ... ;)
        var items = javaBean.match(/(\/\*\*.*?\*\/.*?private \w+ \w+;)|(\/\/.*?private \w+ \w+;)/gi);
        // console.log(items);
        // debugger;

        //遍历数组，获取key和注释放到即将拼接的数组中
        var mainArr = [];  //用于保存主model的配置的数组
        if (null != items && items.length > 0) {
            for (var i = 0; i < items.length; i++) {
                var json = [];
                var item = items[i];
                var comment = item.match(/[^\n\f\r\t\v\*\/ ]+/)[0];  //注释：多个中文或者字符
                var tempArr = item.split(' ');
                var temp = tempArr[tempArr.length - 1];
                var key = temp.substring(0, temp.length - 1);  //键-key
        ...
        var childrenArr = [];  //用于保存从对象的数组
        var modelChildren = '';  //拼接Model生成DetailModel和Collection的字符串
        var relations = '';  //从model对主model的依赖关系

        var allPrivate = javaBean.match(/(\/\*\*.*?\*\/.*?private .*? \w+;)|(\/\/.*?private .*? \w+;)/gi);
        if (null != allPrivate && allPrivate.length > 0) {
            //遍历数组，获取key和注释放到即将拼接的数组中
            for (var k = 0; k < allPrivate.length; k++) {
                //private List<类名而非Map<String,Object>> 键名;
                var children = allPrivate[k].match(/(\/\*\*.*?\*\/.*? private List<\w+> \w+;)|(\/\/.*? private List<\w+> \w+;)/g);
                if (children != null && children.length > 0) {
                    ...
                    var comment = item.match(/[^\n\f\r\t\v\/\* ]+/)[0];  //注释：多个中文或者字符
                    var tempArr = item.split(' ');
                    var tempModelName = tempArr[tempArr.length - 2];
                    //从对象的类名
                    var beanName = tempModelName.substring(5, tempModelName.length - 1);
                    var thisModelName = 'Model' + beanName;
                    var temp = tempArr[tempArr.length - 1];
                    var key = temp.substring(0, temp.length - 1);
                    ...
                    //拼接Model配置和依赖关系
                    // var thisname = childrenArr[j][0];
                    var configJson = ModelName + 'Json';
        ...
        //将修改状态的字符串拼接
        var stateStr = xzStateStr + ckStateStr + chooseState;
...
function contactString(modelJson, mainArr, childrenArr, className, classComment, modelChildren, relations, stateStr) {
    ...
    var ModelName = 'Model' + className;
    var relationStr = (relations != null && relations.length > 0
        ? ',\n' +
        'relations: [\n' +
        relations + '\n' +
        ']'
        : '');
    var relationToMain = '    //创建主model对象\n' +
    ...
    //拼接请求数据的字符串
    var modelObj = className.toLowerCase();
    var getData = '    //获取' + className + '的数据，并赋值给' + modelObj + '\n' +
        'var ' + modelObj + ' = {};\n' +
        'if (id) {  //直接请求获取' + className + '实体中的数据\n' +
        '$.ajax({\n' +
        'type: "post",\n' +
        'url: "/ruixin/get' + className + '",\n' +
        'async: false,\n' +
        'data: {id: id},\n' +
        'success: function (ar) {\n' +
        'if (ar.success) {\n' +
    ...
    //拼接尾部的保存和删除方法
    var modelName = 'model' + className;
    var tailJs = '    //生成model实例对象\n' +
        modelName + ' = new ' + ModelName + '(' + modelObj + ');\n' +
        '    //将' + modelName + '中的数据渲染到表单页面\n' +
        modelName + '.render();\n\n' +
        '});\n\n' +
        '//保存验证\n' +
        ...
        'data: { ' + className[0].toLowerCase() + className.slice(1) + ': ' + modelName + '.getJson() , wid: wid, sid: sid, sort: sort},\n' +
        'async: false,\n' +
        'dataType: "json",\n' +
        'success: function (ar) {\n' +
        'if (ar.success) {\n' +
