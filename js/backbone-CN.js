        ...
        save: function (key, value, options) {
            ...
            options.success = function (resp, status, xhr) {
                ...
                if (!model.set(serverAttrs, options))
                    return false;
                if (success) {
                    // 调用响应成功后自定义的success回调函数                    success(model, resp);
                } else {
        ...
        destroy: function (options) {
            ...
            options.success = function (resp) {
                ...
                if (options.wait)
                    triggerDestroy();
                if (success) {
                    // 调用自定义的成功回调函数                    success(model, resp);
                } else {
...
// 集合中存储的数据模型应该都是同一个模型类的实例        model: Model,
...
// 通过调用history.navigate方法, 手动设置跳转到URL
        navigate: function (fragment, options) {
            // 代理到history实例的navigate方法            Backbone.history.navigate(fragment, options);
        },
...
// originalModel是发生错误的模型或集合对象
    Backbone.wrapError = function (onError, originalModel, options) {
        return function (model, resp) {
            ...
            if (onError) {
                // 如果设置了自定义错误处理方法, 则调用自定义方法                onError(originalModel, resp, options);
            } else {
...
// @param {Object} staticProps 扩展子类的静态属性(或方法)对象
    var inherits = function (parent, protoProps, staticProps) {
        ...
        // 将父类中的静态属性复制为子类静态属性        _.extend(child, parent);
